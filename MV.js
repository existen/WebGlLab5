...
function multVectorWithMatrix(matrix, vec) {
    var result = [];
...
function rotate(angle, axis) {
    ...
    var result = mat4.apply(void 0, vec4(x * x * omc + c, x * y * omc - z * s, x * z * omc + y * s, 0.0).concat(vec4(x * y * omc + z * s, y * y * omc + c, y * z * omc - x * s, 0.0), vec4(x * z * omc - y * s, y * z * omc + x * s, z * z * omc + c, 0.0), vec4()));
    return result;
}
//----------------------------------------------------------------------------
...
function lookAt(eye, at, up) {
    ...
    var v = normalize(subtract(at, eye), false); // view direction vector
    var n = normalize(cross(v, up), false); // perpendicular vector
    var u = normalize(cross(n, v), false); // "new" up vector
    v = negate(v);
    var result = mat4.apply(void 0, vec4.apply(void 0, n.concat([-dot(n, eye)])).concat(vec4.apply(void 0, u.concat([-dot(u, eye)])), vec4.apply(void 0, v.concat([-dot(v, eye)])), vec4()));
    return result;
}
//----------------------------------------------------------------------------
//
